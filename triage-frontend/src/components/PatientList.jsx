...
export default function PatientList({ patients, onStatusUpdate, isHistory = false }) {
  if (!patients || patients.length === 0) {
    return (
      <div className="glass-panel" style={{ height: '100%' }}>
        <h2>{isHistory ? 'Discharged Patients History' : 'Active Triage Queue'}</h2>
        <div className="empty-state">
          <CheckCircle size={48} style={{ marginBottom: '1rem', opacity: 0.5 }} />
          <h3>{isHistory ? 'No History' : 'All clear!'}</h3>
          <p>{isHistory ? 'No patients have been discharged yet.' : 'No patients currently in the active triage queue.'}</p>
        </div>
      </div>
    );
  }
